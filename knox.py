...
def mksecls (): 
  ...
  thalDict = {'TC': (TCsl, TCsl[0].soma[0].gnabar_hh2), 
              'RE': (REsl, REsl[0].soma[0].gnabar_hh2),
              'PY': (PYsl, PYsl[0].soma[0].gnabar_hh2),
...
def setparams (pnafjr=0.0, gnamult=1.0, tyli=['RE', 'TC', 'PY', 'IN']):
  for ty in tyli:
    for ce in thalDict[ty][0]:
      ce.soma[0].gnabar_hh2nafjr = pnafjr *  gnamult * thalDict[ty][1]
      ce.soma[0].gnabar_hh2  =  (1-pnafjr) * gnamult * thalDict[ty][1]
